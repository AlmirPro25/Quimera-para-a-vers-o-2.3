...
const logger = winston.createLogger({
    ...
    transports: [
        new winston.transports.File({ filename: 'error.log', level: 'error', handleExceptions: true, maxsize: 5242880, maxFiles: 5 }),
...
function cacheMiddleware(durationInSeconds) {
    return (req, res, next) => {
        ...
        if (req.query.all === 'true') {
            logger.debug(`[CACHE] Bypass para exportação completa: ${key}`);
            return next();
        }
...
const Agent = sequelize.define('Agent', {
    ...
    status: { type: DataTypes.ENUM('IDLE', 'EXECUTING', 'ERROR', 'OFFLINE', 'PENDING_APPROVAL'), defaultValue: 'OFFLINE' },
    lastSeen: { type: DataTypes.DATE, defaultValue: DataTypes.NOW },
    capabilities: { type: DataTypes.JSONB, allowNull: true }
...
const MissionTemplate = sequelize.define('MissionTemplate', {
    ...
    requiredCapabilities: { type: DataTypes.JSONB, allowNull: true },
...
const MissionExecution = sequelize.define('MissionExecution', {
    id: { type: DataTypes.INTEGER, autoIncrement: true, primaryKey: true },
    agentRecordId: { type: DataTypes.INTEGER, allowNull: false },
    missionTemplateIdFk: { type: DataTypes.INTEGER, allowNull: true },
    manual_objective: { type: DataTypes.STRING, allowNull: true },
    manual_context: { type: DataTypes.JSONB, allowNull: true },
    status: { type: DataTypes.ENUM('PENDING', 'IN_PROGRESS', 'COMPLETED', 'FAILED', 'PENDING_APPROVAL'), defaultValue: 'PENDING' },
...
cortexRouter.post('/analyze',
    ...
    validateRequest,
    async (req, res, next) => { /* ... (cortex logic remains unchanged) ... */ }
);
...
authRouter.post('/login',
    ...
    validateRequest,
    async (req, res, next) => { /* ... (login logic remains unchanged) ... */ }
);
...
authRouter.post('/register',
    ...
    validateRequest,
    async (req, res, next) => { /* ... (register logic remains unchanged) ... */ }
);

apiRouter.use(authenticateJWT);
apiRouter.use('/cortex', cortexRouter);

apiRouter.get('/agents', async (req, res, next) => { /* ... (get agents logic remains unchanged) ... */ });
apiRouter.get('/agents/:agentId',
    [param('agentId').isUUID().withMessage('ID do agente deve ser um UUID válido.')],
    validateRequest,
    async (req, res, next) => { /* ... (get agent by ID logic remains unchanged) ... */ }
);
apiRouter.post('/agents/register',
    ...
    validateRequest,
    async (req, res, next) => { /* ... (register agent logic remains unchanged) ... */ }
);
apiRouter.post('/agents/heartbeat',
    [
        body('agentId').isUUID().withMessage('ID do agente deve ser um UUID válido.'),
        body('status').isIn(['IDLE', 'EXECUTING', 'ERROR', 'PENDING_APPROVAL']).withMessage('Status inválido.').optional(),
        body('capabilities').isObject().withMessage('Capabilities deve ser um objeto válido.').optional()
    ],
    validateRequest,
    async (req, res, next) => { /* ... (heartbeat logic remains unchanged) ... */ }
);

apiRouter.get('/agents/:agentId/task',
    [param('agentId').isUUID().withMessage('ID do agente deve ser um UUID válido.')],
    validateRequest,
    async (req, res, next) => { /* ... (get task logic remains unchanged, but ensure it skips PENDING_APPROVAL missions) ... */ }
);
...
apiRouter.post('/missions/:missionId/update',
    ...
    validateRequest,
    async (req, res, next) => { /* ... (update mission logic remains largely unchanged, ensure AI_INSIGHTS part is there) ... */ }
);

// Campanhas
apiRouter.get('/campaigns', async (req, res, next) => { /* ... (get campaigns logic remains unchanged) ... */ });
apiRouter.get('/campaigns/:campaignId',
    [param('campaignId').isInt().withMessage('ID da campanha deve ser um número inteiro.')],
    validateRequest,
    async (req, res, next) => { /* ... (get campaign by ID logic remains unchanged) ... */ }
);
apiRouter.post('/campaigns',
    ...
    validateRequest,
    async (req, res, next) => { /* ... (create campaign logic remains unchanged) ... */ }
);
apiRouter.put('/campaigns/:campaignId/status',
    ...
    validateRequest,
    async (req, res, next) => { /* ... (update campaign status logic remains unchanged) ... */ }
);
...
apiRouter.post('/campaigns/:campaignId/templates',
    [
        ...
        body('templateType').isIn([
            'WEB_SCRAPE', 'API_MONITORING', 'IMAGE_RECOGNITION', 'SENTIMENT_ANALYSIS',
            'DATA_PROCESSING', 'AI_INSIGHTS', 'SOFTWARE_INSTALLATION', 'SYSTEM_CONFIGURATION',
            'BROWSER_NAVIGATE', 'BROWSER_SCREENSHOT', 'BROWSER_GET_HTML'
        ]).withMessage('Tipo de template inválido.'),
        body('context').isObject().optional().withMessage('Contexto deve ser um objeto JSON válido.').custom(value => {
            try { JSON.parse(JSON.stringify(value)); return true; } catch (e) { throw new Error('Contexto deve ser um JSON válido.'); }
        }),
        body('requiredCapabilities').isObject().optional().withMessage('Capacidades requeridas devem ser um objeto JSON válido.').custom(value => {
            try { JSON.parse(JSON.stringify(value)); return true; } catch (e) { throw new Error('Capacidades requeridas devem ser um JSON válido.'); }
        })
    ],
    validateRequest,
    async (req, res, next) => { /* ... (create template logic remains unchanged, but now supports new types) ... */ }
);
...
apiRouter.post('/dispatch/goal',
    ...
    validateRequest,
    async (req, res, next) => { /* ... (dispatch logic remains unchanged, but needs to handle PENDING_APPROVAL for certain keywords) ... */ }
);

// Analytics
apiRouter.get('/analytics/overview', async (req, res, next) => { /* ... (overview logic unchanged) ... */ });
apiRouter.get('/analytics/campaign/:campaignId/sentiment', cacheMiddleware(30), /* ... */ async (req, res) => { /* ... */ });
apiRouter.get('/analytics/campaign/:campaignId/table-results', cacheMiddleware(30), /* ... */ async (req, res) => { /* ... */ }); // This might be replaced or augmented by the new generic endpoint
apiRouter.get('/analytics/campaign/:campaignId/image-results', cacheMiddleware(60), /* ... */ async (req, res) => { /* ... */ }); // This might be replaced or augmented
apiRouter.get('/analytics/campaign/:campaignId/api-monitoring', cacheMiddleware(15), /* ... */ async (req, res) => { /* ... */ });
apiRouter.get('/analytics/campaign/:campaignId/ai-insights', cacheMiddleware(20), /* ... */ async (req, res, next) => { /* ... (ensure this is correct) ... */ });

// MODIFICATION A.1: Create New Generic Mission Results Endpoint
apiRouter.get('/analytics/campaign/:campaignId/mission-results',
    cacheMiddleware(20),
    [
        param('campaignId').isInt().withMessage('ID da campanha deve ser um número inteiro.'),
        query('type').isString().notEmpty().withMessage('O parâmetro "type" (templateType) é obrigatório.')
    ],
    validateRequest,
    async (req, res, next) => {
        try {
            const { campaignId } = req.params;
            const { type: templateType } = req.query;

            const missions = await MissionExecution.findAll({
                where: {
                    status: 'COMPLETED',
                    '$MissionTemplate.campaignId$': campaignId,
                    '$MissionTemplate.templateType$': templateType,
                    result: { [Op.ne]: null }
                },
                include: [
                    { model: MissionTemplate, attributes: ['templateType', 'objective'] },
                    { model: Agent, attributes: ['agentId', 'hostname'] }
                ],
                order: [['createdAt', 'DESC']],
                attributes: ['id', 'result', 'createdAt', 'manual_objective', 'status']
            });
            ...
            logger.error(`Erro ao buscar resultados de missão para campanha ${req.params.campaignId}, tipo ${req.query.type}: ${error.message}`, { stack: error.stack, userId: req.user.userId });
            next(error);
        }
    }
);
...
server.on('upgrade', (request, socket, head) => { /* ... (WebSocket upgrade logic remains unchanged) ... */ });
wss.on('connection', (ws, req) => { /* ... (WebSocket connection logic remains unchanged) ... */ });
function broadcastToUIs(message) { /* ... (broadcast logic remains unchanged) ... */ }

// Função para semear dados iniciais no banco de dados
    ...
    try {
        ...
        if (campaignCount === 0) {
            ...
            await MissionTemplate.bulkCreate([
                ...
                {
                    campaignId: demoCampaign.id,
                    objective: 'Navegar para Website Exemplo',
                    templateType: 'BROWSER_NAVIGATE',
                    context: { url: 'https://example.com' },
                    requiresApproval: false
                },
                {
                    campaignId: demoCampaign.id,
                    objective: 'Capturar Screenshot do Google',
                    templateType: 'BROWSER_SCREENSHOT',
                    context: { url: 'https://google.com', screenshotOptions: { fullPage: false, type: 'jpeg', quality: 80 } },
                    requiresApproval: false
                },
                {
                    campaignId: demoCampaign.id,
                    objective: 'Extrair HTML do Blog da OpenAI',
                    templateType: 'BROWSER_GET_HTML',
                    context: { url: 'https://openai.com/blog' },
                    requiresApproval: false
                }
            ]);
            logger.info('[DB Seeder] Campanha e templates de demonstração (incluindo Gemini e Browser Control) criados.');
        }
    } catch (error) {
    ...
    try {
        ...
        app.use((err, req, res, next) => {
            logger.error(`[GLOBAL_ERROR_HANDLER] Erro inesperado: ${err.message}`, {
                ...
                errorCode: err.status || res.statusCode
            });
            const statusCode = err.status || 500;
            const message = err.message || 'Ocorreu um erro inesperado. Por favor, tente novamente mais tarde.';
            res.status(statusCode).json({
                error: 'Erro interno do servidor.',
                message: process.env.NODE_ENV === 'production' && statusCode === 500 ? 'Ocorreu um erro inesperado. Por favor, tente novamente mais tarde.' : message
            });
        });
...
startServer();
```
THIS_IS_THE_SEPARATOR_TOKEN_BETWEEN_BACKEND_AND_FRONTEND_CODE
```jsx
...
function CampaignAnalytics({ campaign }) {
    ...
    const renderAnalyticsWidget = () => {
        ...
        switch (viewType) {
            ...
            // MODIFICATION B.5: Integrate New Widgets
    ...
    return (
        <div className="command-panel rounded-lg p-6 space-y-6 fade-in min-h-[500px]" data-aid="div-campaignAnalyticsPanel-cAP6u7">
            ...
            <nav className="flex flex-wrap space-x-1 bg-gray-900/50 rounded-lg p-1" data-aid="nav-analyticsType-nAT0y1">
                ...
                {/* MODIFICATION B.5: Add buttons for new browser widgets */}
...
// MODIFICATION B.1: Update ScreenshotGalleryWidget
...
// MODIFICATION B.2: Update HtmlViewerWidget
function HtmlViewerWidget({ campaignId }) {
    ...
    useEffect(() => {
        // Auto-select first mission if list changes and current selection is gone or not set
        if (missions && missions.length > 0) {
            if (!selectedMissionId || !missions.find(m => m.id === parseInt(selectedMissionId))) {
                setSelectedMissionId(missions[0].id.toString());
            }
        } else {
    ...
    return (
        <div data-aid="div-htmlViewerWidget-hVW0n2">
            ...
            <WidgetStatus loading={loading} error={error} data={missions} dataType="conteúdo HTML capturado">
                <div className="mb-4" data-aid="div-htmlSelectContainer-hSC2p4">
                    ...
                    >
                        ...
                        {missions?.map(mission => (
                            <option key={mission.id} value={mission.id.toString()} data-aid={`option-htmlMission-${mission.id}`}>
                                {mission.result?.source_url || `Missão ${mission.id}`} - {new Date(mission.timestamp).toLocaleString()}
                            </option>
                        ))}
                    </select>
                </div>
                {selectedMission && selectedMission.result?.html_content && (
                 ...
                 {selectedMission && !selectedMission.result?.html_content && (
                    <p className="text-amber-400">Conteúdo HTML não disponível para esta missão.</p>
                )}
            </WidgetStatus>
        </div>
    );
    ...
    const renderCapabilityBadge = (key, value) => {
        ...
        switch (key) {
            ...
            case 'has_screen_access':
                ...
                break;
            // MODIFICATION B.2: Display browser control capability
            case 'has_browser_control':
                iconClass = value ? 'fas fa-window-maximize' : 'fas fa-window-close';
                ...
                <form onSubmit={handleSubmit} className="space-y-4" data-aid="form-newTemplate-fI1w6">
                    ...
                    <div>
                        ...
                        >
                            ...
                            {/* MODIFICATION B.1: Update NewMissionTemplateModal Component */}
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            rows="3"
                            className="w-full p-2 font-mono text-sm rounded-md bg-[var(--input-bg)] border border-[var(--input-border)] focus:ring-2 focus:ring-emerald-500 text-[var(--text-primary)]"
                            placeholder='{ "url": "https://example.com", "selector": "...", "screenshotOptions": {} }' // MODIFICATION B.1: Updated placeholder
                            value={context}
